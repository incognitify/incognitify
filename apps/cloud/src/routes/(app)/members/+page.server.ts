...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  if (!ctx) {
    return { needsOrg: true as const, orgId: null, role: null, selfUserId, members: [], invitations: [] };
  }
  ...
  return { needsOrg: false as const, orgId: ctx.orgId, role: ctx.role, selfUserId, members, invitations };
};
